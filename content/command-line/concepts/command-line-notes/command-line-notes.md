...
The command line takes a program, a command and sometimes a destination in order to perform actions that help navigate and develop the filesystem in your computer. A typical command will look like this:
...
9. rm - removes a file. Add -r to remove a directory and all of it's child elements.

	rm -rf {directory_name} removes the directory and all its contents without prompting for confirmation
    
10. echo - the echo command takes and argument and prints it to the console
11. REDIRECT > - The > character takes the standard output of the command on the left and reciredts it to the file on the right.
...
19. sed stands for 'stream editor.' It accepts standard input and modifies it based on an expression, before displaying it as output data. It is similar to 'find and replace.'
...
2. git status - checks the status of the files being worked on in the working directory
    2.5. git diff check differences between working directory and staging area
...
### Synching
11. Step 1: set up folder in GitHub by creating new repository
12. Step 2: open the terminal (also in VS code)
13. Step 3: set you local project as the current working directory 
...
17. Step 7: copy URL to GitHub repository and type: git remote add origin {link to repository}
...
23. Create a pull request: 
	...
	Step 3: Make changes, commit, then push the branch to repo
	Step 4: From webpage of repo, click "Pull Requests" tab
	Step 5: review pull request
	Step 6: Merge
	Step 7: Delete closed branches
24. Merge
21. Renaming a branch: git branch -m {old-branch-name} {new-branch-name}
22. Deleting a branch: git branch -d {branch-name}, git branch -D if files in conflict
...
26. To synch main branch with upstream main branch: git merge upstream/main
